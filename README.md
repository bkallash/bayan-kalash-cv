# Bayan Kalash — Professional CV

ATS-optimized curriculum vitae for **Bayan Kalash**, Software Engineering student and Full-Stack Developer.

## Files

| File | Purpose |
|------|---------|
| `Bayan_Kalash_CV.pdf` | Ready-to-send PDF (selectable text — ATS-friendly) |
| `Bayan_Kalash_CV.html` | Editable print layout — open in a browser and **Print → Save as PDF** |
| `Bayan_Kalash_CV.md` | Markdown version for GitHub, LinkedIn, or copy-paste |
| `Bayan_Kalash_CV.txt` | Plain-text ATS upload (safest parse for many applicant tracking systems) |

## ATS tips when applying

1. Prefer uploading **`.txt`** or the PDF exported from the HTML (text must be selectable — never a scanned image).
2. Mirror keywords from each job description in the Professional Summary and Skills sections where truthful.
3. Keep the filename clean: `Bayan_Kalash_CV.pdf`.

## Customize before each application

- Swap the Professional Summary’s last sentence to match the role (e.g. Laravel backend, full-stack, NGO/tech for good).
- Add 1–2 project bullets with real links when you have public repos or demos.

Built with [BrainDaemon](https://braindaemon.com).
